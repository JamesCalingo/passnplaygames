const playerArray = ['X - you are the Fake Artist!'];
let category;
...
$('#goArtist').on('click', function (e) {
  e.preventDefault()
  const numOfPlayers = $('.playerNum').val().trim()
  const word = $('#fakeArtistWord').val().trim()
...
$(document).on('click', '.findRole', function (e) {
  ...
  if (playerArray.length === 0) {
    ...
    const randNum = Math.floor(Math.random() * playerArray.length)
    const selection = playerArray[randNum]
    Swal.fire({
      title: `The word is...\n${selection}`,
